...
class timeSeries {
  ...
  // changed if else from dataURL and model URL, what if both are provided
  //reviewed
  async init() {
  ...
  //calls nndata createmetadata, calls add default layers
  //reviewed no idea purpose
  ...
  //calls timeSeries again, nn.model.getweights, setweights
  //reviewed
  copy() {
    const nnCopy = new timeSeries(this.options);
    return tf.tidy(() => {
  ...
  //   // ({inputLabels,outputLabels}=tsUtils.prepareLabels(xInputs, yInputs, options = null));

  //   console.log('raw', xInputs);

  //   const xs = this.searchAndFormat(xInputs);
  //   const xs = nnUtils.formatDataAsObject(formattedInputs, inputLabels);

  //   const ys = nnUtils.formatDataAsObject(yInputs, outputLabels);

  //   console.log('xs and yx', xInputs);
  //   console.log('xs and yx',ys)
  //   //create formatted input first, since the data is time series, the format of the data should be the following
  //   /* [
  //         {
  //           xs:[{x: ,y: },{x: ,y: },{x: ,y: },{x: ,y: }], 
  //           ys: {'label': }
  ...
  //   this.neuralNetworkData.addData(xInputs, ys);
  // }
  addData(xInputs, yInputs, options = null) {
  ...
  formatInputItem(input) {
    ...
    console.log("formatted input item", formattedInputs)
    return formattedInputs;
  }
